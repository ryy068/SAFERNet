...
class Attention(nn.Module):
    def __init__(self, dim, window_size=None, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.token_interaction = Conv2d_BN(dim//3, dim//3, 5, 1, 5//2, groups=dim//3, resolution=14)
...
class StokenAttention(nn.Module):
    def __init__(self, dim, stoken_size=to_2tuple(8), n_iter=3, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
    ...
    def stoken_forward(self, x, hw_shape):
        '''
           x: (B, C, H, W)
        '''
        B, _, C = x.shape
        ...
        pixel_features = torch.transpose(pixel_features, 1, 2) # B,C,L -> B,L,C
    ...
    def forward(self, x, hw_shape):
        if self.stoken_size[0] > 1 or self.stoken_size[1] > 1:
            return self.stoken_forward(x,hw_shape)
        else:
